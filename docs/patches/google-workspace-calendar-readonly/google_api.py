...
from __future__ import annotations  # allow PEP 604 annotations on Python 3.9+
...
# It is primary-calendar-only, today/tomorrow by default, and uses a strict
# fields mask. Do not use full calendar scope for this workflow.
CALENDAR_SAFE_LIST_FIELDS = "items(summary,start,end,location)"
...
def _safe_window_today_tomorrow() -> tuple[str, str]:
    local_now = datetime.now().astimezone()
    today_start = local_now.replace(hour=0, minute=0, second=0, microsecond=0)
    after_tomorrow_start = today_start + timedelta(days=2)
    return today_start.isoformat(), after_tomorrow_start.isoformat()


def _calendar_safe_event(e: dict) -> dict:
    start_obj = e.get("start", {})
    end_obj = e.get("end", {})
    all_day = "date" in start_obj and "date" in end_obj
    return {
...
def calendar_safe_list(args):
    # Safety boundary: Phase 2 assistant calendar reads must use this command only.
    # It always reads only the primary calendar and requests only the fields in
    # CALENDAR_SAFE_LIST_FIELDS. Do not broaden this for the Phase 2 workflow.
    default_start, default_end = _safe_window_today_tomorrow()
    time_min = _datetime_with_timezone(args.start or default_start)
    time_max = _datetime_with_timezone(args.end or default_end)
    ...
    params = {
        ...
        "singleEvents": True,
        "orderBy": "startTime",
        "fields": CALENDAR_SAFE_LIST_FIELDS,
    }
    ...
    events = [_calendar_safe_event(e) for e in results.get("items", [])]
...
def main():
    ...
    p = cal_sub.add_parser("safe-list", help="Phase 2 safe read: primary calendar, today/tomorrow by default, strict fields mask")
    p.add_argument("--start", default="", help="Optional start time override (ISO 8601); defaults to local today 00:00")
    p.add_argument("--end", default="", help="Optional end time override (ISO 8601); defaults to local day-after-tomorrow 00:00")
    p.add_argument("--max", type=int, default=25)
    p.set_defaults(func=calendar_safe_list)
