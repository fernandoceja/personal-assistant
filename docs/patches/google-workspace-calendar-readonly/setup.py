...
def scopes_for_services(services_value: str | None) -> list[str]:
    ...
    unknown = [s for s in requested if s not in SERVICE_SCOPES]
    if unknown:
...
SCOPES = scopes_for_services("all")
...
def get_auth_url(services: str | None = None):
    ...
    try:
        requested_scopes = scopes_for_services(services)
    ...
    _save_pending_auth(state=state, code_verifier=flow.code_verifier, requested_scopes=requested_scopes)
...
def exchange_auth_code(code: str):
    ...
    missing_scopes = _missing_scopes_from_payload(token_payload, granted_scopes)
    if missing_scopes:
